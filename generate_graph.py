...
def compute_rrg(sector_close: pd.Series,
    ...
    # rs_sma = rs.ewm(span=rs_period, adjust=False).mean()
    # rs_sma = rs.ewm(alpha=1/rs_period, adjust=False).mean()
...
output_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "rrg_output.png")
